...
export default function AccountDashboard() {
  ...
  useEffect(() => {
    ...
    async function loadReservations() {
      ...
      try {
        ...
        console.error('Error fetching dashboard reservations:', err);
      } finally {
  ...
  const upcomingReservations = reservations.filter(
    (r) => r.status === 'confirmed' || r.status === 'pending' || r.status === 'checked_in'
  );
  ...
  return (
    <div>
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <h2 className="heading-4" style={{ marginBottom: 'var(--space-md)' }}>
          Próximas reservas
        </h2>
